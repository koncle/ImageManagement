...
public class TagDao extends AbstractDao<Tag, Long> {
    ...
    private Query<Tag> image_TagsQuery;
    ...
    public List<Tag> _queryImage_Tags(Long image_id) {
        synchronized (this) {
            if (image_TagsQuery == null) {
                QueryBuilder<Tag> queryBuilder = queryBuilder();
                queryBuilder.join(TagAndImage.class, TagAndImageDao.Properties.Tag_id)
                    .where(TagAndImageDao.Properties.Image_id.eq(image_id));
                image_TagsQuery = queryBuilder.build();
            }
        }
        Query<Tag> query = image_TagsQuery.forCurrentThread();
